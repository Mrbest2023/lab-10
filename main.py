...
offload_url = 'http://172.20.10.1:5000' # TODO: Change this to the IP address of your server
...
def run(offload: Optional[str] = None) -> float:
    ...
    if offload is None: # in this case, we run the program locally
        ...
        def offload_process1(data):
            ...
            r=requests.post("http://offload_url/process1", data={'foo':'bar'})
            data1 = response.json()
        ...
        def offload_process2(data):
            nonlocal data2
            r_2=requests.post("http://offload_url/process2", data={'foo':'bar'})
            data2=response.json()
